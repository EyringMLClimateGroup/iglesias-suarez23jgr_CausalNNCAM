...
varnm_3d_list = ['qbp','qcbp','qibp','tbp','vbp','phq','phcldliq','phcldice','tphystnd','qrl','qrs','dtvke',
                 ...
                 'rh','bmse']
varnm_2d_list = ['ps','solin','shflx','lhflx','fsnt','fsns','flnt','flns','prect','prectend','precst','precsten',
                 'lhf_nsdelq']
...
var_3d_dict = {i.lower() : np.zeros([nTimes,len(var_idx_dict[i.lower()]),nLats,nLons])  for i in var_list if len(var_idx_dict[i.lower()]) > 1 or i.lower() in varnm_3d_list}
var_3d_dict.keys()

# #### 2-D variables (zero arrays)
var_2d_dict = {i.lower() : np.zeros([nTimes,len(var_idx_dict[i.lower()]),nLats,nLons])  for i in var_list if len(var_idx_dict[i.lower()]) == 1 and i.lower() in varnm_2d_list}
var_2d_dict.keys()
